...
ВАЖЛИВО — пошуковий fallback вбудований у проксі (smartSearch): він автоматично пробує оригінал → нормалізований → без ТОВ/ФОП → по окремих словах. Тобі достатньо передати повну назву як є — проксі сам знайде найкращий збіг.
